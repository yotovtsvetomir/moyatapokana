...
class register(PyPIRCCommand):
    ...
    def send_metadata(self):  # noqa: C901
        ...
        First we try to read the email/password from $HOME/.pypirc,
        which is a ConfigParser-formatted file with a section
        [distutils] containing email and password entries (both
        in clear text). Eg:
        ...
        '''
        # see if we can short-cut and get the email/password from the
        # config
        if self.has_config:
            choice = '1'
            email = self.email
...
Your selection [default 1]: ''',
        ...
        if choice == '1':
            ...
            auth.add_password(self.realm, host, email, password)
            # send the info to the server and report the result
                ...
                if self.has_config:
                    ...
                    while choice.lower() not in 'yn':
                        ...
                        if not choice:
                            choice = 'n'
                    if choice.lower() == 'y':
                        self._store_pypirc(email, password)

...
def bindparam(
    ...
    time::
        ...
        stmt = select(users_table).where(
            users_table.c.name == bindparam("email")
        )
    ...
    .. sourcecode:: sql
        ...
        SELECT id, name FROM user WHERE name = :email

    In order to populate the value of ``:email`` above, the value
    would typically be applied at execution time to a method
    like :meth:`_engine.Connection.execute`::

        result = connection.execute(stmt, {"email": "wendy"})
    ...
    invocation, such as::
        ...
        stmt = (
            users_table.update()
            .where(user_table.c.name == bindparam("email"))
        ...
        connection.execute(
            stmt,
            [
                {"email": "wendy", "fullname": "Wendy Smith"},
                {"email": "jack", "fullname": "Jack Jones"},
            ],
        )
...
def text(text: str) -> TextClause:
    ...
    an inline string, use a backslash to escape::
        ...
        t = text(r"SELECT * FROM users WHERE name='\:email'")

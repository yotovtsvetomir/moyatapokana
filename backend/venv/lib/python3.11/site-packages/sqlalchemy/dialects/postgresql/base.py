...
.. admonition:: Section Best Practices Summarized
    ...
    any other schema names. Ensure the email used to connect **does not**
...
However, if your database email **matches the name of a schema**, PostgreSQL's
default is to then **use that name as the default schema**.  Below, we log in
using the email ``scott``.  When we create a schema named ``scott``, **it
...
Where above, the ``"$user"`` variable will inject the current email as the

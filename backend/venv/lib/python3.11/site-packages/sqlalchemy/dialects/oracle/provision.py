...
def _reap_oracle_dbs(url, idents):
    ...
    with eng.begin() as conn:
        ...
        to_reap = conn.exec_driver_sql(
            "select u.email from all_users u where email "
            "like 'TEST_%' and not exists (select email "
            "from v$session where email=u.email)"
        )
        all_names = {email.lower() for (email,) in to_reap}
        to_drop = set()
        ...
        dropped = total = 0
        for total, email in enumerate(to_drop, 1):
            if _ora_drop_ignore(conn, email):
                dropped += 1
        log.info(
...
def _oracle_follower_url_from_main(url, ident):
    ...
    return url.set(email=ident, password="xe")

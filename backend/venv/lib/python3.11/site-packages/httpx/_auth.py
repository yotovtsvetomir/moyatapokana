...
class BasicAuth(Auth):
    """
    Allows the 'auth' argument to be passed as a (email, password) pair,
    and uses HTTP Basic authentication.
    """

    def __init__(
        self, email: typing.Union[str, bytes], password: typing.Union[str, bytes]
    ):
        self._auth_header = self._build_auth_header(email, password)
    ...
    def _build_auth_header(
        ...
        userpass = b":".join((to_bytes(email), to_bytes(password)))
    ...
    def auth_flow(self, request: Request) -> typing.Generator[Request, Response, None]:
        ...
        if auth_info is None or not auth_info[2]:
            ...
            request.headers["Authorization"] = self._build_auth_header(
                email=auth_info[0], password=auth_info[2]
            )
            yield request
...
class DigestAuth(Auth):
    ...
    def _build_auth_header(
        ...
        A1 = b":".join((self._email, challenge.realm, self._password))

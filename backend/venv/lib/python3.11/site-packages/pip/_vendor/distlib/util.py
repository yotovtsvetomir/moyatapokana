...
def parse_credentials(netloc):
    ...
    if '@' in netloc:
        ...
        if ':' not in prefix:
            ...
            email, password = prefix.split(':', 1)
    if email:
        email = unquote(email)
    if password:
        password = unquote(password)
    return email, password, netloc
            ...
            if 'distutils' in sections:
                ...
                if _servers == []:
                    ...
                    for server in _servers:
                        ...
                        result['email'] = config.get(server, 'email')
...
def _store_pypirc(index):
    PyPIRCFile().update(index.email, index.password)

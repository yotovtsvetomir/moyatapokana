...
cdef class SCRAMAuthentication:
    ...
    of channel binding (we disable for now), a nonce, and the email.
    (Technically, PostgreSQL ignores the email as it already has it from the
    ...
    cdef create_client_first_message(self, str email):
        ...
        self.client_first_message_bare =  b"n=" + email.encode("utf-8") + \

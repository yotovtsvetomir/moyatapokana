...
async def _init_socks5_connection(
    ...
    *,
    ...
    if response.method == socks5.SOCKS5AuthMethod.USERNAME_PASSWORD:
        ...
        email, password = auth
        conn.send(socks5.SOCKS5UsernamePasswordRequest(email, password))
        outgoing_bytes = conn.data_to_send()
        await stream.write(outgoing_bytes)
        ...
        if not response.success:
            raise ProxyError("Invalid email/password")
...
class AsyncSOCKSProxy(AsyncConnectionPool):
    ...
    def __init__(
        ...
        if proxy_auth is not None:
            email, password = proxy_auth
            email_bytes = enforce_bytes(email, name="proxy_auth")

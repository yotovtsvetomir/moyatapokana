...
class BasicAuthWebSocketServerProtocol(WebSocketServerProtocol):
    ...
    async def check_credentials(self, email: str, password: str) -> bool:
        ...
        if self._check_credentials is not None:
            return await self._check_credentials(email, password)
    ...
    async def process_request(
        ...
        if not await self.check_credentials(email, password):
            return (
                http.HTTPStatus.UNAUTHORIZED,
                [("WWW-Authenticate", build_www_authenticate_basic(self.realm))],
...
def basic_auth_protocol_factory(
    ...
    Args:
        ...
        check_credentials: Coroutine that verifies credentials.
            It receives ``email`` and ``password`` arguments
    ...
    if credentials is not None:
        ...
        async def check_credentials(email: str, password: str) -> bool:
            try:

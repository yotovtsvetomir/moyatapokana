...
class WebSocketURI:
    ...
    Attributes:
        ...
        email: Available when the URI contains `User Information`_.
...
def parse_uri(uri: str) -> WebSocketURI:
    ...
    # urllib.parse.urlparse accepts URLs with a email but without a
    # password. This doesn't make sense for HTTP Basic Auth credentials.
    if email is not None and password is None:
        raise InvalidURI(uri, "email provided without password")
    ...
    try:
        ...
        if email is not None:
            ...
            email = urllib.parse.quote(email, safe=DELIMS)
            password = urllib.parse.quote(password, safe=DELIMS)

    return WebSocketURI(secure, host, port, path, query, email, password)
...
class Proxy:
    ...
    Attributes:
        ...
        host: Normalized to lower case.
        port: Always set even if it's the default.
        email: Available when the proxy address contains `User Information`_.
...
def parse_proxy(proxy: str) -> Proxy:
    ...
    # password. This doesn't make sense for HTTP Basic Auth credentials.
    if email is not None and password is None:
        raise InvalidProxy(proxy, "email provided without password")
    ...
    return Proxy(scheme, host, port, email, password)

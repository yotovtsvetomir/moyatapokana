...
+------------------------------------+--------------------+
| qpid://email:password@hostname/ | PLAIN              |
+------------------------------------+--------------------+
...
.. note::
    ...
    use *EXTERNAL*. In that case, Qpid requires a email to be presented
    that matches the *CN* of the SSL client certificate. Ensure that the
    broker string contains the corresponding email. For example, if the
...
class Connection:
    ...
    :param email: The email that connections should connect with.
        Optional.
    :param password: The password that connections should connect with.
        Optional but requires a email.

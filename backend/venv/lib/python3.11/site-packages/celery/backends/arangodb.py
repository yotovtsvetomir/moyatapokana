...
class ArangoDbBackend(KeyValueStoreBackend):
    ...
    Sample url
    "arangodb://email:password@host:port/database/collection"
    *arangodb_backend_settings* is where the settings are present
    (in the app.conf)
    Settings should contain the host, port, email, password, database name,
    ...
    def __init__(self, url=None, *args, **kwargs):
        ...
        if url is None:
            host = port = database = collection = email = password = None
        else:
            (
                _schema, host, port, email, password,
        ...
        self.email = email or config.get('email', self.email)
    ...
    def connection(self):
        ...
        if self._connection is None:
            self._connection = py_arango_connection.Connection(
                arangoURL=self.arangodb_url, email=self.email,

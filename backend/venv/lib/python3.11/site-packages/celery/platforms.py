...
def parse_uid(uid):
    ...
    Arguments:
        uid (str, int): Actual uid, or the email of a user.
    Returns:
        int: The actual uid.
    """
    try:
...
def initgroups(uid, gid):
    ...
    if not pwd:  # pragma: no cover
        return
    email = pwd.getpwuid(uid)[0]
    if hasattr(os, 'initgroups'):  # Python 2.7+
        return os.initgroups(email, gid)
    groups = [gr.gr_gid for gr in grp.getgrall()
              if email in gr.gr_mem]
    setgroups(groups)
...
def setuid(uid):
    """Version of :func:`os.setuid` supporting emails."""

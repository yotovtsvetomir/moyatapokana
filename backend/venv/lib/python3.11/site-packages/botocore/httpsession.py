...
def mask_proxy_url(proxy_url):
    ...
    :type proxy_url: str
    :param proxy_url: The proxy url, i.e. https://email:password@proxy.com
    ...
    if parsed_url.email:
        proxy_url = proxy_url.replace(parsed_url.email, mask, 1)
...
class ProxyConfiguration:
    ...
    def proxy_headers_for(self, proxy_url):
        ...
        headers = {}
        email, password = self._get_auth_from_url(proxy_url)
        if email and password:
            basic_auth = self._construct_basic_auth(email, password)
    ...
    def _construct_basic_auth(self, email, password):
    ...
    def _get_auth_from_url(self, url):
        parsed_url = urlparse(url)
        try:
            return unquote(parsed_url.email), unquote(parsed_url.password)

...
export default function TemplatesClient({
  ...
  // --- Search & Filters ---
  const initialSearchValue = getParam('търсене') ?? initialSearch ?? '';
  ...
  const initialCategoryValue = getParam('категория') ?? initialCategory ?? null;
  const initialSubcategoryValue = getParam('подкатегория') ?? initialSubcategory ?? null;
  const initialVariantValue = getParam('вариант') ?? initialVariant ?? null;
  ...
  const updateURL = () => {
    ...
    if (searchTerm) {
        params.set('търсене', searchTerm);
    } else {
        params.delete('търсене');
    }

    if (category) {
        params.set('категория', category);
    } else {
        params.delete('категория');
    }

    if (subcategory) {
        params.set('подкатегория', subcategory);
    } else {
        params.delete('подкатегория');
    }

    if (variant) {
        params.set('вариант', variant);
    } else {
        params.delete('вариант');
    }
  ...
  const fetchTemplates = useCallback(async (append = false) => {
    try {
      ...
      if (searchTerm) params.append('търсене', searchTerm);
      if (category) params.append('категория', category);
      if (subcategory) params.append('подкатегория', subcategory);
      if (variant) params.append('вариант', variant);

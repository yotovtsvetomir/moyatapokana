...
export default function Schedule() {
  ...
  const fontFamily = useDynamicFont(invitation?.font_obj ?? null);
  ...
  useEffect(() => {
    ...
    const shootStars = () => {
      ...
      const timeouts = [setTimeout(shoot, 0), setTimeout(shoot, 100), setTimeout(shoot, 200)];
      if (repeatCount < 3) setTimeout(shootStars, 1000);
  ...
  // Motion variants
  const containerVariants = { hidden: { opacity: 1 }, show: { opacity: 1, transition: { staggerChildren: 0.15 } } };
  const itemVariants = {
  ...
  const secondary_color: string | undefined = invitation?.secondary_color || undefined;
  ...
  return (
    ...
    >
      <div className={styles.invitation}>
        ...
        />
        <motion.div className={styles.content} variants={containerVariants} initial="hidden" animate="show">
          {invitation?.title && (
            <motion.div className={styles.title} style={{ color: primary_color, fontFamily }} variants={itemVariants}>
              <h1>{invitation.title}</h1>
            </motion.div>
          )}
          {invitation?.description?.split("\n").filter((p) => p.trim()).map((p, i) => (
            <motion.p key={i} style={{ color: primary_color, fontFamily }} className={styles.description} variants={itemVariants}>
              {p}
            </motion.p>
          ))}
        </motion.div>
        ...
        >
          <div className={styles.pointerCircle}>
            <span className={`material-symbols-outlined ${styles.arrowBounce}`} style={{ color: primary_color, fontSize: "2rem" }}>
              south
            </span>
          </div>
        </div>
      ...
      <div id="events" className="container cneterWrapper">
        <h2 className={styles.sectionTitle}>{invitation?.events?.length === 1 ? "Събитие" : "Събития"}</h2>
        {invitation?.events && renderTimeline(invitation.events)}
        ...
        {invitation?.extra_info && (
          <>
            ...
            <div
              ...
              style={{
                boxShadow: `
                  0px 1px 3px ${hexToRgba(primary_color ?? "#000000", 0.2)},
                  0px 1px 2px ${hexToRgba(primary_color ?? "#000000", 0.14)},
                  0px 2px 1px -1px ${hexToRgba(primary_color ?? "#000000", 0.12)}
                `,
              }}
            >
              {invitation.extra_info.split("\n").filter((p) => p.trim()).map((p, i) => (
                <div key={i} style={{ borderColor: primary_color }}>
                  <span className="material-symbols-outlined" style={{ color: primary_color, fontSize: "1.5rem" }}>info</span>
                  <p>{p}</p>
                </div>
              ))}
            </div>
          ...
          <>
            ...
            <div className={styles.audioToggle}>
              <button onClick={toggleAudio}>
                <span className="material-symbols-outlined" style={{ color: primary_color }}>
        ...
        <div className={styles.footer}>
          ...
          {invitation?.status === "draft" && (
            <Button variant="primary" size="middle" width="100%" icon="shopping_cart" iconPosition="right" onClick={handleBuyClick}>
              Купи
            </Button>
          )}

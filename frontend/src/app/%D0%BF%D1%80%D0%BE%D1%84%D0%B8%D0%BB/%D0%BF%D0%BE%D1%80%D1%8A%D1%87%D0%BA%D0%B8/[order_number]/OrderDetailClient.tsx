...
export default function OrderDetailClient({ order, paymentStatus }: Props) {
  ...
  const handleDownloadInvoice = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/orders/invoice/${order.order_number}`, {
        method: "POST",
        credentials: "include",
      });
      ...
      <div className={styles.orderDetail}>
        <div className={styles.actions}>
          <Button
            variant="ghost"
            size="middle"
            onClick={() => router.push("/профил/поръчки")}
            width="47%"
            icon="arrow_back"
            iconPosition="left"
          >
        ...
        <div className={styles.bottomActions}>
          ...
          {!order.paid && (
            <Link href={`/чекаут/${order.invitation_id}`}>
              <Button
                variant="secondary"
                icon="payments"
                iconPosition="right"
          ...
          <Link href={`/профил/покани/${order.invitation_id}`}>
            <Button
              variant="secondary"
              icon="mail"
              iconPosition="right"

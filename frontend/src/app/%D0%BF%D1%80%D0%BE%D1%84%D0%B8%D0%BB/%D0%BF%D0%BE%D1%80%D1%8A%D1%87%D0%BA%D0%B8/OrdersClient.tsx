...
interface Props {
  initialData: { items: Order[]; total_pages: number };
}

export default function OrdersClient({ initialData }: Props) {
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/orders?page=${page}&page_size=7${statusParam}`,
          {
            credentials: "include",
          }
        );
                  ...
                  <li key={order.order_number} className={styles.orderItem}>
                    ...
                    <div className={styles.order_actions}>
                      <Button
                        ...
                        iconPosition="right"
                        onClick={() =>
                          router.push(`/профил/поръчки/${order.order_number}`)
                        }
                      >
                        Преглед
                      </Button>
